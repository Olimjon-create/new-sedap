...
function Search() {
  ...
  if (typeof window !== "undefined") {
    ...
    // Xatoliklarni oldini olish uchun JSON.parse ni try/catch bilan tekshiramiz
    if (storedUser && storedUser !== "undefined") {
      try {
        user = JSON.parse(storedUser);
      } catch (error) {
        console.error("Failed to parse user from localStorage:", error);
        localStorage.removeItem("user"); // noto‘g‘ri format bo‘lsa, tozalaymiz
      }
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span style={{ fontWeight: "400" }}>Hello Samantha</span>
          {user ? `, ${user.username}` : ""}
        </h3>
